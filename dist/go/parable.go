...
func (self *Lexer) Peek() string {
	...
	return _runeAt(self.Source, self.Pos)
}
...
func (self *Lexer) Advance() string {
	...
	c := _runeAt(self.Source, self.Pos)
	self.Pos++
	return c
}
...
func (self *Lexer) readOperator() *Token {
	...
	if c == "<" {
		if self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "(" {
			return nil
		}
		self.Pos++
...
func (self *Lexer) skipComment() bool {
	...
	if _runeAt(self.Source, self.Pos) != "#" {
		return false
	}
	...
	if self.Pos > 0 {
		prev := _runeAt(self.Source, self.Pos-1)
		if !strings.Contains(" \t\n;|&(){}", prev) {
			return false
		}
	}
	for self.Pos < self.Length && _runeAt(self.Source, self.Pos) != "\n" {
		self.Pos++
	}
	return true
}
...
func (self *Lexer) isWordTerminator(ctx int, ch string, bracketDepth int, parenDepth int) bool {
	if ctx == WORDCTXREGEX {
		if ch == "]" && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "]" {
			return true
		}
		if ch == "&" && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "&" {
			return true
...
func (self *Lexer) readBracketExpression(chars *[]string, parts []Node, forRegex bool, parenDepth int) bool {
	if forRegex {
		scan := self.Pos + 1
		if scan < self.Length && _runeAt(self.Source, scan) == "^" {
			scan++
		}
		if scan < self.Length && _runeAt(self.Source, scan) == "]" {
			scan++
		...
		for scan < self.Length {
			sc := _runeAt(self.Source, scan)
			if sc == "]" && scan+1 < self.Length && _runeAt(self.Source, scan+1) == "]" {
				break
			}
			if sc == ")" && parenDepth > 0 {
				break
			...
			if sc == "[" && scan+1 < self.Length && _runeAt(self.Source, scan+1) == ":" {
				scan += 2
				for scan < self.Length && !(_runeAt(self.Source, scan) == ":" && scan+1 < self.Length && _runeAt(self.Source, scan+1) == "]") {
					scan++
				}
				if scan < self.Length {
					scan += 2
		...
		nextCh := _runeAt(self.Source, self.Pos+1)
	...
	*chars = append(*chars, self.Advance())
	...
	for !self.AtEnd() {
		...
		if c == "[" && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == ":" {
			*chars = append(*chars, self.Advance())
			*chars = append(*chars, self.Advance())
			for !self.AtEnd() && !(self.Peek() == ":" && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "]") {
				*chars = append(*chars, self.Advance())
			}
			if !self.AtEnd() {
				*chars = append(*chars, self.Advance())
		...
		} else if !forRegex && c == "[" && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "=" {
			*chars = append(*chars, self.Advance())
			*chars = append(*chars, self.Advance())
			for !self.AtEnd() && !(self.Peek() == "=" && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "]") {
				*chars = append(*chars, self.Advance())
		...
		if ch == "$" && !self.AtEnd() && !((flags & MatchedPairFlagsEXTGLOB) != 0) {
			...
			if nextCh == "{" {
				if (flags & MatchedPairFlagsARITH) != 0 {
					...
					if afterBracePos >= self.Length || !isFunsubChar(_runeAt(self.Source, afterBracePos)) {
						chars = append(chars, ch)
						wasDollar = true
						wasGtlt = false
						continue
				...
				var cmdNode Node
				var cmdText string
				if self.Pos+2 < self.Length && _runeAt(self.Source, self.Pos+2) == "(" {
...
func (self *Lexer) readWordInternal(ctx int, atCommandStart bool, inArrayLiteral bool, inAssignBuiltin bool) *Word {
	...
	for !self.AtEnd() {
		...
		if ctx != WORDCTXREGEX && ch == "$" && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "'" {
		...
		if ctx != WORDCTXREGEX && isRedirectChar(ch) && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "(" {
			self.syncToParser()
		...
		if self.extglob && ctx == WORDCTXNORMAL && isExtglobPrefix(ch) && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "(" {
			chars = append(chars, self.Advance())
			chars = append(chars, self.Advance())
			content = self.parseMatchedPair("(", ")", MatchedPairFlagsEXTGLOB, false)
			chars = append(chars, content)
...
func (self *Lexer) readWord() *Token {
	...
	isProcsub := (c == "<" || c == ">") && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "("
...
func (self *Lexer) readLocaleString() (Node, string, []Node) {
	...
	for !self.AtEnd() {
		...
		} else if ch == "$" && self.Pos+2 < self.Length && _runeAt(self.Source, self.Pos+1) == "(" && _runeAt(self.Source, self.Pos+2) == "(" {
			self.syncToParser()
			arithNode, arithText := self.parser.parseArithmeticExpansion()
			self.syncFromParser()
			if arithNode != nil {
...
func (self *Lexer) consumeParamName() string {
	...
	if isSpecialParam(ch) {
		if ch == "$" && self.Pos+1 < self.Length && strings.Contains("{'\"", _runeAt(self.Source, self.Pos+1)) {
			return ""
		}
		self.Advance()
		return ch
...
func (self *Lexer) readBracedParam(start int, inDquote bool) (Node, string) {
	...
	op = self.consumeParamOperator()
	if op == "" {
		if !self.AtEnd() && self.Peek() == "$" && self.Pos+1 < self.Length && (_runeAt(self.Source, self.Pos+1) == "\"" || _runeAt(self.Source, self.Pos+1) == "'") {
			...
			self.Advance()
			op = "`"
		} else if !self.AtEnd() && self.Peek() == "$" && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "{" {
			op = ""
	...
	if (op == "<" || op == ">") && strings.HasPrefix(arg, "(") && strings.HasSuffix(arg, ")") {
		...
		func() {
			...
			if !_isNilInterfaceRef(parsed) && subParser.AtEnd() {
		...
		if _runeAt(inner, i) == "\\" && i+1 < _runeLen(inner) {
			...
			if simple >= 0 {
				...
				if i+2 < _runeLen(inner) && _runeAt(inner, i+2) == "{" {
					j := i + 3
					for j < _runeLen(inner) && isHexDigit(_runeAt(inner, j)) {
						j++
					}
					...
					j := i + 2
					for j < _runeLen(inner) && j < i+4 && isHexDigit(_runeAt(inner, j)) {
						j++
					}
					if j > i+2 {
						...
						result = append(result, byte(int(_runeAt(inner, i)[0])))
						i++
					}
				}
			} else if c == "u" {
				j := i + 2
				for j < _runeLen(inner) && j < i+6 && isHexDigit(_runeAt(inner, j)) {
					j++
				}
				if j > i+2 {
				...
				j := i + 2
				for j < _runeLen(inner) && j < i+10 && isHexDigit(_runeAt(inner, j)) {
					j++
				}
				if j > i+2 {
					codepoint := _parseInt(substring(inner, i+2, j), 16)
				...
				j := i + 2
				for j < _runeLen(inner) && j < i+4 && isOctalDigit(_runeAt(inner, j)) {
					j++
				}
				if j > i+2 {
...
func (self *Word) expandAllAnsiCQuotes(value string) string {
	...
	for i < _runeLen(value) {
		...
		if ch == "'" && !effectiveInDquote {
			isAnsiC := !quote.Single && i > 0 && _runeAt(value, i-1) == "$" && (countConsecutiveDollarsBefore(value, i-1)%2) == 0
			if !isAnsiC {
				quote.Single = !quote.Single
			}
			result = append(result, ch)
...
func (self *Word) stripLocaleStringDollars(value string) string {
	...
	for i < _runeLen(value) {
		...
		if ch == "\\" && i+1 < _runeLen(value) && !quote.Single && !braceQuote.Single {
			...
			result = append(result, _runeAt(value, i+1))
			i += 2
		} else if startsWithAt(value, i, "${") && !quote.Single && !braceQuote.Single && (i == 0 || _runeAt(value, i-1) != "$") {
			braceDepth++
...
func (self *Word) normalizeArrayWhitespace(value string) string {
	i := 0
	if !(i < _runeLen(value) && (_strIsAlpha(_runeAt(value, i)) || _runeAt(value, i) == "_")) {
		return value
	}
	i++
	for i < _runeLen(value) && (_strIsAlnum(_runeAt(value, i)) || _runeAt(value, i) == "_") {
		i++
	}
...
func (self *Word) formatCommandSubstitutions(value string, inArith bool) string {
	...
	for idx < _runeLen(value) {
		if _runeAt(value, idx) == "\"" {
			...
			if idx == 0 || !_strIsAlnum(_runeAt(value, idx-1)) && !strings.Contains("\"'", _runeAt(value, idx-1)) {
	...
	arithParenDepth := 0
	for i < _runeLen(value) {
		if i > 0 && isExtglobPrefix(_runeAt(value, i-1)) && _runeAt(value, i) == "(" && !isBackslashEscaped(value, i-1) {
			extglobDepth++
			result = append(result, _runeAt(value, i))
			i++
			continue
		...
		if startsWithAt(value, i, "$(") && !startsWithAt(value, i, "$((") && !isBackslashEscaped(value, i) && !isDollarDollarParen(value, i) {
			...
			cmdsubIdx++
			i = j
		} else if isExpansionStart(value, i, "${") && i+2 < _runeLen(value) && isFunsubChar(_runeAt(value, i+2)) && !isBackslashEscaped(value, i) {
			j = findFunsubEnd(value, i+2)
			var cmdsubNode Node = func() Node {
				if cmdsubIdx < len(cmdsubParts) {
			...
			isProcsub := i == 0 || !_strIsAlnum(_runeAt(value, i-1)) && !strings.Contains("\"'", _runeAt(value, i-1))
			if extglobDepth > 0 {
				j = findCmdsubEnd(value, i+2)
				result = append(result, substring(value, i, j))
			...
			if procsubIdx < len(procsubParts) {
				...
				func() {
					...
					parser = NewParser(inner, false, false)
					parsed = parser.ParseList(true)
					if !_isNilInterfaceRef(parsed) && parser.Pos == _runeLen(inner) && !strings.Contains(inner, "\n") {
...
func (self *Redirect) ToSexp() string {
	...
	if strings.HasPrefix(op, "{") {
		j := 1
		if j < _runeLen(op) && (_strIsAlpha(_runeAt(op, j)) || _runeAt(op, j) == "_") {
			j++
			for j < _runeLen(op) && (_strIsAlnum(_runeAt(op, j)) || _runeAt(op, j) == "_") {
				j++
			}
...
func (self *CasePattern) ToSexp() string {
	...
	for i < _runeLen(self.Pattern) {
		ch := _runeAt(self.Pattern, i)
		if ch == "\\" && i+1 < _runeLen(self.Pattern) {
			current = append(current, substring(self.Pattern, i, i+2))
			i += 2
		} else if (ch == "@" || ch == "?" || ch == "*" || ch == "+" || ch == "!") && i+1 < _runeLen(self.Pattern) && _runeAt(self.Pattern, i+1) == "(" {
...
func (self *Parser) lexPeekReservedWord() string {
	...
	if _mapHas(RESERVEDWORDS, word) || word == "{" || word == "}" || word == "[[" || word == "]]" || word == "!" || word == "time" {
		return word
	}
	return ""
}
...
func (self *Parser) Advance() string {
	...
	ch := _runeAt(self.Source, self.Pos)
	self.Pos++
	return ch
}
...
func (self *Parser) PeekAt(offset int) string {
	...
	return _runeAt(self.Source, pos)
}
...
func (self *Parser) isBangFollowedByProcsub() bool {
	...
	nextChar := _runeAt(self.Source, self.Pos+1)
	if nextChar != ">" && nextChar != "<" {
		return false
	}
	return _runeAt(self.Source, self.Pos+2) == "("
}
...
func (self *Parser) atListTerminatingBracket() bool {
	...
	if ch == "}" {
		...
		return isWordEndContext(_runeAt(self.Source, nextPos))
	}
	return false
}
...
func (self *Parser) parseLoopBody(context string) Node {
	...
	if self.lexConsumeWord("do") {
		var body Node = self.ParseListUntil(map[string]struct{}{"done": {}})
		if _isNilInterfaceRef(body) {
...
func (self *Parser) ConsumeWord(expected string) bool {
	...
	if word != "" && self.inProcessSub && _runeLen(word) > 1 && _runeAt(word, 0) == "}" {
	...
	for self.Peek() == "\\" && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "\n" {
		self.Advance()
		self.Advance()
	}
	return true
...
func (self *Parser) parseDollarExpansion(chars *[]string, parts *[]Node, inDquote bool) bool {
	var result0 Node
	var result1 string
	if self.Pos+2 < self.Length && _runeAt(self.Source, self.Pos+1) == "(" && _runeAt(self.Source, self.Pos+2) == "(" {
...
func (self *Parser) isAssignmentWord(word *Word) bool {
...
func (self *Parser) parseBacktickSubstitution() (Node, string) {
	...
	for !self.AtEnd() && (inHeredocBody || self.Peek() != "`") {
		if inHeredocBody {
			lineStart := self.Pos
			lineEnd := lineStart
			for lineEnd < self.Length && _runeAt(self.Source, lineEnd) != "\n" {
				lineEnd++
			}
			...
			if checkLine == currentHeredocDelim {
				...
				self.Pos = lineEnd
				if self.Pos < self.Length && _runeAt(self.Source, self.Pos) == "\n" {
		...
		if e := recover(); e != nil {
			...
			contentStartChar := func() string {
				if start+2 < self.Length {
					return _runeAt(self.Source, start+2)
				} else {
					return ""
				}
			}()
...
func (self *Parser) arithPeek(offset int) string {
	...
	return _runeAt(self.arithSrc, pos)
}
...
func (self *Parser) arithAdvance() string {
	...
	c := _runeAt(self.arithSrc, self.arithPos)
	self.arithPos++
	return c
}
...
func (self *Parser) arithSkipWs() {
	for !self.arithAtEnd() {
		...
		if isWhitespace(c) {
			self.arithPos++
		} else if c == "\\" && self.arithPos+1 < self.arithLen && _runeAt(self.arithSrc, self.arithPos+1) == "\n" {
			self.arithPos += 2
		} else {
			break
		}
...
func (self *Parser) arithParseComma() Node {
	var left Node = self.arithParseAssign()
	for true {
		self.arithSkipWs()
		if self.arithConsume(",") {
			self.arithSkipWs()
			var right Node = self.arithParseAssign()
...
func (self *Parser) arithParseAssign() Node {
	var left Node = self.arithParseTernary()
	self.arithSkipWs()
	assignOps := []string{"<<=", ">>=", "+=", "-=", "*=", "/=", "%=", "&=", "^=", "|=", "="}
...
func (self *Parser) arithParseTernary() Node {
	var cond Node = self.arithParseLogicalOr()
	self.arithSkipWs()
	if self.arithConsume("?") {
		self.arithSkipWs()
		var ifTrue Node
...
func (self *Parser) arithParseBitwiseOr() Node {
	var left Node = self.arithParseBitwiseXor()
	for true {
		self.arithSkipWs()
		if self.arithPeek(0) == "|" && self.arithPeek(1) != "|" && self.arithPeek(1) != "=" {
			self.arithAdvance()
			self.arithSkipWs()
			var right Node = self.arithParseBitwiseXor()
...
func (self *Parser) arithParseBitwiseXor() Node {
	var left Node = self.arithParseBitwiseAnd()
	for true {
		self.arithSkipWs()
		if self.arithPeek(0) == "^" && self.arithPeek(1) != "=" {
			self.arithAdvance()
			self.arithSkipWs()
			var right Node = self.arithParseBitwiseAnd()
			left = &ArithBinaryOp{Op: "^", Left: left, Right: right, Kind: "binary-op"}
		} else {
			break
		}
...
func (self *Parser) arithParseBitwiseAnd() Node {
	var left Node = self.arithParseEquality()
	for true {
		self.arithSkipWs()
		if self.arithPeek(0) == "&" && self.arithPeek(1) != "&" && self.arithPeek(1) != "=" {
			self.arithAdvance()
			self.arithSkipWs()
			var right Node = self.arithParseEquality()
			left = &ArithBinaryOp{Op: "&", Left: left, Right: right, Kind: "binary-op"}
		} else {
			break
		}
...
func (self *Parser) arithParseComparison() Node {
	var left Node = self.arithParseShift()
	for true {
		self.arithSkipWs()
		var right Node
		if self.arithMatch("<=") {
...
func (self *Parser) arithParseShift() Node {
	var left Node = self.arithParseAdditive()
	for true {
		self.arithSkipWs()
		if self.arithMatch("<<=") {
			break
...
func (self *Parser) arithParseAdditive() Node {
	var left Node = self.arithParseMultiplicative()
	for true {
		self.arithSkipWs()
		c := self.arithPeek(0)
		c2 := self.arithPeek(1)
...
func (self *Parser) arithParseMultiplicative() Node {
	var left Node = self.arithParseExponentiation()
	for true {
		self.arithSkipWs()
		c := self.arithPeek(0)
		c2 := self.arithPeek(1)
...
func (self *Parser) arithParseExponentiation() Node {
	var left Node = self.arithParseUnary()
	self.arithSkipWs()
	if self.arithMatch("**") {
		self.arithConsume("**")
		self.arithSkipWs()
		var right Node = self.arithParseExponentiation()
...
func (self *Parser) arithParsePostfix() Node {
	var left Node = self.arithParsePrimary()
	for true {
		self.arithSkipWs()
		if self.arithMatch("++") {
			self.arithConsume("++")
			...
			if leftVar, ok := left.(*ArithVar); ok {
				self.arithAdvance()
				self.arithSkipWs()
				var index Node = self.arithParseComma()
				self.arithSkipWs()
				if !self.arithConsume("]") {
...
func (self *Parser) arithParsePrimary() Node {
	...
	if c == "(" {
		self.arithAdvance()
		self.arithSkipWs()
		var expr Node = self.arithParseComma()
		self.arithSkipWs()
		if !self.arithConsume(")") {
...
func (self *Parser) arithParseCmdsub() Node {
	...
	if self.arithPeek(0) == "(" {
		...
		self.arithAdvance()
		self.arithAdvance()
		var innerExpr Node = self.parseArithExpr(content)
...
func (self *Parser) arithParseNumberOrVar() Node {
	...
	if _strIsDigit(c) {
		...
		if !self.arithAtEnd() && self.arithPeek(0) == "$" {
			var expansion Node = self.arithParseExpansion()
...
func (self *Parser) ParseRedirect() Node {
	...
	if fd == -1 && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "(" {
		self.Pos = start
		return nil
	}
	op = self.Advance()
	...
	if !self.AtEnd() {
		...
		if op == ">" && nextCh == ">" {
			...
			if self.Pos+1 >= self.Length || !isDigitOrDash(_runeAt(self.Source, self.Pos+1)) {
				self.Advance()
				op = ">&"
			}
		} else if fd == -1 && varfd == "" && op == "<" && nextCh == "&" {
	...
	if !self.AtEnd() && self.Peek() == "&" {
		...
		self.SkipWhitespace()
		if !self.AtEnd() && self.Peek() == "-" {
			if self.Pos+1 < self.Length && !isMetachar(_runeAt(self.Source, self.Pos+1)) {
				self.Advance()
...
func (self *Parser) parseHeredocDelimiter() (string, bool) {
	...
	for true {
		...
		for !self.AtEnd() && !isMetachar(self.Peek()) {
			...
			if ch == "\"" {
			...
			} else if ch == "$" && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "'" {
				quoted = true
				self.Advance()
				self.Advance()
				for !self.AtEnd() && self.Peek() != "'" {
				...
				dollarCount := 0
				j := self.Pos - 1
				for j >= 0 && _runeAt(self.Source, j) == "$" {
					dollarCount++
					j--
				}
				if j >= 0 && _runeAt(self.Source, j) == "\\" {
					dollarCount--
				}
				if (dollarCount % 2) == 1 {
					delimiterChars = append(delimiterChars, self.Advance())
		...
		if !self.AtEnd() && strings.Contains("<>", self.Peek()) && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "(" {
			delimiterChars = append(delimiterChars, self.Advance())
			delimiterChars = append(delimiterChars, self.Advance())
			depth = 1
			for !self.AtEnd() && depth > 0 {
...
func (self *Parser) ParseArithmeticCommand() *ArithmeticCommand {
	self.SkipWhitespace()
	if self.AtEnd() || self.Peek() != "(" || self.Pos+1 >= self.Length || _runeAt(self.Source, self.Pos+1) != "(" {
		return nil
	}
	savedPos := self.Pos
	self.Advance()
	...
	for !self.AtEnd() && depth > 0 {
		...
		if c == "'" {
			...
			if depth == 1 && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == ")" {
				break
			}
			depth--
			if depth == 0 {
	...
	self.Advance()
	self.Advance()
	var expr Node = self.parseArithExpr(content)
...
func (self *Parser) ParseConditionalExpr() *ConditionalExpr {
	...
	nextPos := self.Pos + 2
	if nextPos < self.Length && !(isWhitespace(_runeAt(self.Source, nextPos)) || _runeAt(self.Source, nextPos) == "\\" && nextPos+1 < self.Length && _runeAt(self.Source, nextPos+1) == "\n") {
		return nil
	}
	self.Advance()
	self.Advance()
	...
	var body Node = self.parseCondOr()
	for !self.AtEnd() && isWhitespaceNoNewline(self.Peek()) {
		self.Advance()
	}
	if self.AtEnd() || self.Peek() != "]" || self.Pos+1 >= self.Length || _runeAt(self.Source, self.Pos+1) != "]" {
...
func (self *Parser) condSkipWhitespace() {
	for !self.AtEnd() {
		if isWhitespaceNoNewline(self.Peek()) {
			self.Advance()
		} else if self.Peek() == "\\" && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "\n" {
			self.Advance()
			self.Advance()
		} else if self.Peek() == "\n" {
			self.Advance()
...
func (self *Parser) condAtEnd() bool {
	return self.AtEnd() || self.Peek() == "]" && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "]"
}

func (self *Parser) parseCondOr() Node {
	self.condSkipWhitespace()
	var left Node = self.parseCondAnd()
	self.condSkipWhitespace()
	if !self.condAtEnd() && self.Peek() == "|" && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "|" {
		self.Advance()
		self.Advance()
		var right Node = self.parseCondOr()
...
func (self *Parser) parseCondAnd() Node {
	self.condSkipWhitespace()
	var left Node = self.parseCondTerm()
	self.condSkipWhitespace()
	if !self.condAtEnd() && self.Peek() == "&" && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "&" {
		self.Advance()
		self.Advance()
		var right Node = self.parseCondAnd()
...
func (self *Parser) parseCondTerm() Node {
	...
	if self.Peek() == "!" {
		if self.Pos+1 < self.Length && !isWhitespaceNoNewline(_runeAt(self.Source, self.Pos+1)) {
		} else {
			self.Advance()
			var operand Node = self.parseCondTerm()
			return &CondNot{Operand: operand, Kind: "cond-not"}
		}
	}
	if self.Peek() == "(" {
		self.Advance()
		var inner Node = self.parseCondOr()
		self.condSkipWhitespace()
		if self.AtEnd() || self.Peek() != ")" {
	...
	self.condSkipWhitespace()
	if _mapHas(CONDUNARYOPS, word1.Value) {
		unaryOperand := self.parseCondWord()
		if unaryOperand == nil {
	...
	if !self.condAtEnd() && self.Peek() != "&" && self.Peek() != "|" && self.Peek() != ")" {
		var word2 *Word
		if isRedirectChar(self.Peek()) && !(self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "(") {
			op := self.Advance()
			self.condSkipWhitespace()
		...
		if opWord != nil && _mapHas(CONDBINARYOPS, opWord.Value) {
			self.condSkipWhitespace()
			if opWord.Value == "=~" {
...
func (self *Parser) ParseIf() *If {
	...
	var condition Node = self.ParseListUntil(map[string]struct{}{"then": {}})
	if _isNilInterfaceRef(condition) {
	...
	var thenBody Node = self.ParseListUntil(map[string]struct{}{"elif": {}, "else": {}, "fi": {}})
	if _isNilInterfaceRef(thenBody) {
	...
	if self.lexIsAtReservedWord("elif") {
		...
		var elifCondition Node = self.ParseListUntil(map[string]struct{}{"then": {}})
		if _isNilInterfaceRef(elifCondition) {
		...
		var elifThenBody Node = self.ParseListUntil(map[string]struct{}{"elif": {}, "else": {}, "fi": {}})
		if _isNilInterfaceRef(elifThenBody) {
			panic(NewParseError("Expected commands after 'then'", self.lexPeekToken().Pos, 0))
		}
		self.SkipWhitespaceAndNewlines()
		var innerElse Node
		if self.lexIsAtReservedWord("elif") {
			...
			if _isNilInterfaceRef(innerElse) {
		...
		if _isNilInterfaceRef(elseBody) {
			panic(NewParseError("Expected commands after 'else'", self.lexPeekToken().Pos, 0))
		}
	}
	self.SkipWhitespaceAndNewlines()
...
func (self *Parser) ParseWhile() *While {
	...
	var condition Node = self.ParseListUntil(map[string]struct{}{"do": {}})
	if _isNilInterfaceRef(condition) {
...
func (self *Parser) ParseFor() Node {
	...
	self.SkipWhitespace()
	if self.Peek() == "(" && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "(" {
...
func (self *Parser) parseForArith() *ForArith {
	...
	for !self.AtEnd() {
		...
		if ch == "(" {
			...
			if parenDepth > 0 {
				parenDepth--
				current = append(current, self.Advance())
			} else if self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == ")" {
	...
	self.SkipWhitespaceAndNewlines()
	var body Node = self.parseLoopBody("for loop")
...
func (self *Parser) ParseSelect() *Select {
	...
	self.SkipWhitespaceAndNewlines()
	var body Node = self.parseLoopBody("select")
	...
	for true {
		...
		for !self.AtEnd() {
			...
			if ch == ")" {
				...
				patternChars = append(patternChars, self.Advance())
				extglobDepth++
			} else if self.extglob && isExtglobPrefix(ch) && self.Pos+1 < self.Length && _runeAt(self.Source, self.Pos+1) == "(" {
				patternChars = append(patternChars, self.Advance())
				patternChars = append(patternChars, self.Advance())
				extglobDepth++
			} else if ch == "[" {
				...
				if scanPos < self.Length && isCaretOrBang(_runeAt(self.Source, scanPos)) {
					scanPos++
				}
				if scanPos < self.Length && _runeAt(self.Source, scanPos) == "]" {
				...
				for scanPos < self.Length {
					sc := _runeAt(self.Source, scanPos)
					if sc == "]" && scanDepth == 0 {
						isCharClass = true
						break
					} else if sc == "[" {
...
func (self *Parser) ParseCoproc() *Coproc {
	...
	if nextWord != "" && _mapHas(COMPOUNDKEYWORDS, nextWord) {
	...
	if len(potentialName) > 0 {
		...
		if isValidIdentifier(potentialName) {
			...
			} else if nextWord != "" && _mapHas(COMPOUNDKEYWORDS, nextWord) {
				name = potentialName
				body = self.ParseCompoundCommand()
				if !_isNilInterfaceRef(body) {
					return &Coproc{Command: body, Name: name, Kind: "coproc"}
...
func (self *Parser) ParseFunction() *Function {
	...
	name = self.PeekWord()
	if name == "" || _mapHas(RESERVEDWORDS, name) {
		return nil
	}
...
func (self *Parser) atListUntilTerminator(stopWords map[string]struct{}) bool {
	...
	if self.Peek() == "}" {
		nextPos := self.Pos + 1
		if nextPos >= self.Length || isWordEndContext(_runeAt(self.Source, nextPos)) {
			return true
		}
	}
	reserved := self.lexPeekReservedWord()
	if reserved != "" && _mapHas(stopWords, reserved) {
		return true
	}
...
func (self *Parser) ParseListUntil(stopWords map[string]struct{}) Node {
	...
	if _isNilInterfaceRef(pipeline) {
		return nil
	}
	parts := []Node{pipeline}
	for true {
...
func (self *Parser) ParseCompoundCommand() Node {
	...
	if reserved == "" && self.inProcessSub {
		...
		if word != "" && _runeLen(word) > 1 && _runeAt(word, 0) == "}" {
			keywordWord := _Substring(word, 1, _runeLen(word))
			if _mapHas(RESERVEDWORDS, keywordWord) || keywordWord == "{" || keywordWord == "}" || keywordWord == "[[" || keywordWord == "]]" || keywordWord == "!" || keywordWord == "time" {
...
func (self *Parser) ParsePipeline() Node {
	...
	if self.lexIsAtReservedWord("time") {
		...
		if !self.AtEnd() && startsWithAt(self.Source, self.Pos, "--") {
			if self.Pos+2 >= self.Length || isWhitespace(_runeAt(self.Source, self.Pos+2)) {
				self.Advance()
				self.Advance()
				timePosix = true
				self.SkipWhitespace()
		...
		self.SkipWhitespace()
		if !self.AtEnd() && self.Peek() == "!" {
			if (self.Pos+1 >= self.Length || isNegationBoundary(_runeAt(self.Source, self.Pos+1))) && !self.isBangFollowedByProcsub() {
				self.Advance()
	...
	var result Node = self.parseSimplePipeline()
	if prefixOrder == "time" {
		...
		result = &Time{Pipeline: result, Posix: timePosix, Kind: "time"}
		result = &Negation{Pipeline: result, Kind: "negation"}
	} else if _isNilInterfaceRef(result) {
		return nil
	}
	return result
}
...
func (self *Parser) stripTrailingBackslashFromLastWord(nodes []Node) {
	...
	var lastNode Node = nodes[len(nodes)-1]
...
func (self *Parser) findLastWord(node Node) *Word {
	...
	case *Command:
		...
		if len(node.Redirects) > 0 {
			var lastRedirect Node = node.Redirects[len(node.Redirects)-1]
...
func formatCmdsubNode(node Node, indent int, inProcsub bool, compactRedirects bool, procsubFirst bool) string {
	...
	switch node := node.(type) {
	case *Negation:
		if !_isNilInterfaceRef(node.Pipeline) {
...
func formatHeredocBody(r *HereDoc) string {
	return "\n" + r.Content + r.Delimiter + "\n"
}
...
func skipHeredoc(value string, start int) int {
	...
	for i < _runeLen(value) && isWhitespaceNoNewline(_runeAt(value, i)) {
		i++
	}
	delimStart := i
	...
	if i < _runeLen(value) && (_runeAt(value, i) == "\"" || _runeAt(value, i) == "'") {
		...
		for i < _runeLen(value) && !isMetachar(_runeAt(value, i)) {
			i++
		}
		delimiter = substring(value, delimStart, i)
	} else {
...
func findHeredocContentEnd(source string, start int, delimiters []struct {
	...
	pos := start
	for pos < _runeLen(source) && _runeAt(source, pos) != "\n" {
		pos++
	}
	...
	for _, item := range delimiters {
		...
		for pos < _runeLen(source) {
			lineStart := pos
			lineEnd := pos
			for lineEnd < _runeLen(source) && _runeAt(source, lineEnd) != "\n" {
				lineEnd++
			}
...
func isWordBoundary(s string, pos int, wordLen int) bool {
	...
	if end < _runeLen(s) && (_strIsAlnum(_runeAt(s, end)) || _runeAt(s, end) == "_") {
		return false
	}
	return true
}
...
func isArrayAssignmentPrefix(chars []string) bool {
	...
	i := 1
	for i < _runeLen(s) && (_strIsAlnum(_runeAt(s, i)) || _runeAt(s, i) == "_") {
		i++
	}
	for i < _runeLen(s) {
		if _runeAt(s, i) != "[" {
