...
class ParseContext {
  ...
  copy() {
    let ctx = newParseContext(this.kind);
...
class Lexer {
  ...
  ReadOperator() {
    ...
    let two = this.lookahead(2);
    let three = this.lookahead(3);
    if (three === ";;&") {
      this.pos += 3;
  ...
  ReadBracketExpression(chars, parts, forRegex, parenDepth) {
    if (forRegex) {
      ...
      let bracketWillClose = false;
      while (scan < this.length) {
        let sc = this.source[scan];
      ...
      let nextCh = this.source[this.pos + 1];
  ...
  ParseMatchedPair(openChar, closeChar, flags, initialWasDollar) {
    ...
    let chars = [];
    let passNext = false;
    let wasDollar = initialWasDollar;
    let wasGtlt = false;
    while (count > 0) {
      if (this.atEnd()) {
      ...
      if ("'\"`".includes(ch) && openChar !== closeChar) {
        ...
        if (ch === "'") {
          chars.push(ch);
          let quoteFlags = (wasDollar ? flags | MatchedPairFlags_ALLOWESC : flags);
      ...
      if (ch === "$" && !this.atEnd() && !((flags & MatchedPairFlags_EXTGLOB) !== 0)) {
        ...
        if (nextCh === "{") {
          ...
          this.pos -= 1;
          this.SyncToParser();
          let inDquote = (flags & MatchedPairFlags_DQUOTE) !== 0;
          let [paramNode, paramText] = this.Parser.ParseParamExpansion(inDquote);
          this.SyncFromParser();
          if (paramNode !== null) {
      ...
      if (ch === "(" && wasGtlt && ((flags & (MatchedPairFlags_DOLBRACE | MatchedPairFlags_ARRAYSUB)) !== 0)) {
        let direction = chars[chars.length - 1];
        chars = chars.slice(0, chars.length - 1);
        this.pos -= 1;
        this.SyncToParser();
        let [procsubNode, procsubText] = this.Parser.ParseProcessSubstitution();
        this.SyncFromParser();
        if (procsubNode !== null) {
  ...
  ReadWordInternal(ctx, atCommandStart, inArrayLiteral, inAssignBuiltin) {
    ...
    while (!this.atEnd()) {
      ...
      if ((ctx === WORD_CTX_COND || ctx === WORD_CTX_REGEX) && ch === "[") {
        let forRegex = ctx === WORD_CTX_REGEX;
      ...
      if (ch === "'") {
        this.advance();
        let trackNewline = ctx === WORD_CTX_NORMAL;
        let sawNewline;
        ...
        if (ctx === WORD_CTX_NORMAL) {
          ...
          while (!this.atEnd() && (inSingleInDquote || this.peek() !== "\"")) {
            ...
            if (c === "\\" && this.pos + 1 < this.length) {
              let nextC = this.source[this.pos + 1];
              if (nextC === "\n") {
                this.advance();
                this.advance();
              } else {
          ...
          let handleLineContinuation = ctx === WORD_CTX_COND;
          this.SyncToParser();
      ...
      if (ctx !== WORD_CTX_REGEX && ch === "$" && this.pos + 1 < this.length && this.source[this.pos + 1] === "'") {
        let [ansiResult0, ansiResult1] = this.ReadAnsiCQuote();
        if (ansiResult0 !== null) {
      ...
      if (ctx !== WORD_CTX_REGEX && ch === "$" && this.pos + 1 < this.length && this.source[this.pos + 1] === "\"") {
        let [localeResult0, localeResult1, localeResult2] = this.ReadLocaleString();
      ...
      if (ctx !== WORD_CTX_REGEX && IsRedirectChar(ch) && this.pos + 1 < this.length && this.source[this.pos + 1] === "(") {
        this.SyncToParser();
        let [procsubResult0, procsubResult1] = this.Parser.ParseProcessSubstitution();
        this.SyncFromParser();
      ...
      if (ctx === WORD_CTX_NORMAL && ch === "(" && (chars.length > 0) && bracketDepth === 0) {
        ...
        if (isArrayAssign && (atCommandStart || inAssignBuiltin)) {
          this.SyncToParser();
          let [arrayResult0, arrayResult1] = this.Parser.ParseArrayLiteral();
          this.SyncFromParser();
  ...
  ReadWord() {
    ...
    let isProcsub = (c === "<" || c === ">") && this.pos + 1 < this.length && this.source[this.pos + 1] === "(";
    let isRegexParen = this.WordContext === WORD_CTX_REGEX && (c === "(" || c === ")");
    if (this.isMetachar(c) && !isProcsub && !isRegexParen) {
      return null;
    }
    let word = this.ReadWordInternal(this.WordContext, this.AtCommandStart, this.InArrayLiteral, this.InAssignBuiltin);
    if (word === null) {
      return null;
    }
  ...
  peekToken() {
    if (this.TokenCache === null) {
      let savedLast = this.LastReadToken;
  ...
  ReadAnsiCQuote() {
    ...
    let text = Substring(this.source, start, this.pos);
    let content = contentChars.join("");
    let node = new AnsiCQuote(content, "ansi-c");
    return [node, text];
  }
      ...
      if (ch === "\"") {
        ...
        if (ch === "\\" && this.pos + 1 < this.length) {
          ...
          if (ch === "$" && this.pos + 2 < this.length && this.source[this.pos + 1] === "(" && this.source[this.pos + 2] === "(") {
            this.SyncToParser();
            let [arithNode, arithText] = this.Parser.ParseArithmeticExpansion();
            this.SyncFromParser();
            if (arithNode !== null) {
            ...
            if (IsExpansionStart(this.source, this.pos, "$(")) {
              ...
              if (ch === "$") {
                this.SyncToParser();
                let [paramNode, paramText] = this.Parser.ParseParamExpansion(false);
                this.SyncFromParser();
                if (paramNode !== null) {
      ...
      while (!this.atEnd()) {
        ...
        if (/^[a-zA-Z0-9]+$/.test(c) || c === "_") {
          ...
          if (c === "[") {
            ...
            nameChars.push(this.advance());
            let content = this.ParseMatchedPair("[", "]", MatchedPairFlags_ARRAYSUB, false);
  ...
  ReadParamExpansion(inDquote) {
    ...
    if (/^[a-zA-Z]+$/.test(ch) || ch === "_") {
      ...
      let name = Substring(this.source, nameStart, this.pos);
      text = Substring(this.source, start, this.pos);
  ...
  ReadBracedParam(start, inDquote) {
    ...
    let savedDolbrace = this.DolbraceState;
    ...
    if (ch === "!") {
      ...
      if ((param.length > 0)) {
        ...
        if (!this.atEnd() && IsAtOrStar(this.peek())) {
          ...
          let trailing = this.ParseMatchedPair("{", "}", MatchedPairFlags_DOLBRACE, false);
          text = Substring(this.source, start, this.pos);
          this.DolbraceState = savedDolbrace;
    ...
    if (!(param.length > 0)) {
      if (!this.atEnd() && ("-=+?".includes(this.peek()) || this.peek() === ":" && this.pos + 1 < this.length && IsSimpleParamOp(this.source[this.pos + 1]))) {
        ...
        let content = this.ParseMatchedPair("{", "}", MatchedPairFlags_DOLBRACE, false);
        text = "${" + content + "}";
        this.DolbraceState = savedDolbrace;
    ...
    if (op === "") {
      if (!this.atEnd() && this.peek() === "$" && this.pos + 1 < this.length && (this.source[this.pos + 1] === "\"" || this.source[this.pos + 1] === "'")) {
        let dollarCount = 1 + CountConsecutiveDollarsBefore(this.source, this.pos);
        if (dollarCount % 2 === 1) {
          op = "";
        } else {
          op = this.advance();
    ...
    try {
      let flags = (inDquote ? MatchedPairFlags_DQUOTE : MatchedPairFlags_NONE);
      let paramEndsWithDollar = param !== "" && param.endsWith("$");
    ...
    if ((op === "<" || op === ">") && arg.startsWith("(") && arg.endsWith(")")) {
      let inner = arg.slice(1, arg.length - 1);
      try {
        let subParser = newParser(inner, true, this.Parser.Extglob);
        let parsed = subParser.parseList(true);
        if (parsed !== null && subParser.atEnd()) {
          let formatted = FormatCmdsubNode(parsed, 0, true, false, true);
          arg = "(" + formatted + ")";
        }
      } catch (_e) {
      }
...
class Word {
      ...
      if (c === "'" && !quote.double) {
        ...
        if (c === "\"" && !quote.single) {
          ...
          if (IsExpansionStart(value, i, "${") && !quote.single) {
            ...
            result.push("{");
            i += 2;
            let hadLeadingNewline = i < value.length && value[i] === "\n";
          ...
          if (c === "'") {
            ...
            if (c === "x") {
              if (i + 2 < inner.length && inner[i + 2] === "{") {
                ...
                let hexStr = Substring(inner, i + 3, j);
              ...
              if (c === "u") {
                ...
                if (c === "U") {
                  ...
                  if (c === "c") {
                    if (i + 3 <= inner.length) {
                      let ctrlChar = inner[i + 2];
                      let skipExtra = 0;
                      if (ctrlChar === "\\" && i + 4 <= inner.length && inner[i + 3] === "\\") {
                        skipExtra = 1;
                      }
                      let ctrlVal = ctrlChar.charCodeAt(0) & 31;
                      if (ctrlVal === 0) {
                        return result;
                      }
  ...
  ExpandAnsiCEscapes(value) {
    ...
    let inner = Substring(value, 1, value.length - 1);
    let literalBytes = this.AnsiCToBytes(inner);
    let literalStr = new TextDecoder().decode(new Uint8Array(literalBytes));
  ...
  ExpandAllAnsiCQuotes(value) {
    ...
    while (i < value.length) {
      ...
      let effectiveInDquote = quote.double;
      if (ch === "'" && !effectiveInDquote) {
        let isAnsiC = !quote.single && i > 0 && value[i - 1] === "$" && CountConsecutiveDollarsBefore(value, i - 1) % 2 === 0;
        if (!isAnsiC) {
          quote.single = !quote.single;
        }
        result.push(ch);
        ...
        if (ch === "\"" && !quote.single) {
          ...
          if (ch === "\\" && i + 1 < value.length && !quote.single) {
            ...
            if (StartsWithAt(value, i, "$'") && !quote.single && !effectiveInDquote && CountConsecutiveDollarsBefore(value, i) % 2 === 0) {
              ...
              let ansiStr = Substring(value, i, j);
              let expanded = this.ExpandAnsiCEscapes(Substring(ansiStr, 1, ansiStr.length));
              let outerInDquote = quote.outerDouble();
              if (braceDepth > 0 && outerInDquote && expanded.startsWith("'") && expanded.endsWith("'")) {
                let inner = Substring(expanded, 1, expanded.length - 1);
                if (inner.indexOf("\u0001") === -1) {
                  let resultStr = result.join("");
                  let inPattern = false;
                  let lastBraceIdx = resultStr.lastIndexOf("${");
                  if (lastBraceIdx >= 0) {
                    let afterBrace = resultStr.slice(lastBraceIdx + 2);
                    let varNameLen = 0;
                    ...
                    if (varNameLen > 0 && varNameLen < afterBrace.length && !"#?-".includes(afterBrace[0])) {
                      let opStart = afterBrace.slice(varNameLen);
                      ...
                      if (varNameLen === 0 && afterBrace.length > 1) {
                        let firstChar = afterBrace[0];
                        if (!"%#/^,".includes(firstChar)) {
                          let rest = afterBrace.slice(1);
                          for (const op of ["//", "%%", "##", "/", "%", "#", "^", "^^", ",", ",,"]) {
                            if (rest.includes(op)) {
                              inPattern = true;
                              break;
  ...
  StripLocaleStringDollars(value) {
    ...
    let bracketDepth = 0;
    let quote = newQuoteState();
    let braceQuote = newQuoteState();
    let bracketInDoubleQuote = false;
    while (i < value.length) {
      let ch = value[i];
                      ...
                      if (ch === "\"" && !quote.single && !braceQuote.single && braceDepth > 0) {
                        ...
                        if (ch === "'" && !quote.double && !braceQuote.double && braceDepth > 0) {
                          ...
                          if (StartsWithAt(value, i, "$\"") && !quote.single && !braceQuote.single && (braceDepth > 0 || bracketDepth > 0 || !quote.double) && !braceQuote.double && !bracketInDoubleQuote) {
                            let dollarCount = 1 + CountConsecutiveDollarsBefore(value, i);
                            if (dollarCount % 2 === 1) {
                              result.push("\"");
                              if (bracketDepth > 0) {
  ...
  NormalizeArrayWhitespace(value) {
    ...
    let prefix = Substring(value, 0, i + 1);
    let openParenPos = i + 1;
    var closeParenPos;
    if (value.endsWith(")")) {
    ...
    let inner = Substring(value, openParenPos + 1, closeParenPos);
    let suffix = Substring(value, closeParenPos + 1, value.length);
    let result = this.NormalizeArrayInner(inner);
  ...
  FormatCommandSubstitutions(value, inArith) {
    ...
    let hasBraceCmdsub = value.indexOf("${ ") !== -1 || value.indexOf("${\t") !== -1 || value.indexOf("${\n") !== -1 || value.indexOf("${|") !== -1;
    let hasUntrackedCmdsub = false;
    let hasUntrackedProcsub = false;
    let idx = 0;
    let scanQuote = newQuoteState();
    ...
    let hasParamWithProcsubPattern = value.includes("${") && (value.includes("<(") || value.includes(">("));
    ...
    let cmdsubIdx = 0;
    let procsubIdx = 0;
    let mainQuote = newQuoteState();
    let extglobDepth = 0;
    let deprecatedArithDepth = 0;
    let arithDepth = 0;
    let arithParenDepth = 0;
    while (i < value.length) {
      ...
      if (StartsWithAt(value, i, "$(") && !StartsWithAt(value, i, "$((") && !IsBackslashEscaped(value, i) && !IsDollarDollarParen(value, i)) {
        ...
        if (value[i] === "`" && cmdsubIdx < cmdsubParts.length) {
          ...
          if (IsExpansionStart(value, i, "${") && i + 2 < value.length && IsFunsubChar(value[i + 2]) && !IsBackslashEscaped(value, i)) {
            ...
            let cmdsubNode = (cmdsubIdx < cmdsubParts.length ? cmdsubParts[cmdsubIdx] : null);
            if (cmdsubNode instanceof CommandSubstitution && cmdsubNode.brace) {
              node = cmdsubNode;
              formatted = FormatCmdsubNode(node.command, 0, false, false, false);
              let hasPipe = value[i + 2] === "|";
              prefix = (hasPipe ? "${|" : "${ ");
              let origInner = Substring(value, i + 2, j - 1);
              let endsWithNewline = origInner.endsWith("\n");
              var suffix;
            ...
            if ((StartsWithAt(value, i, ">(") || StartsWithAt(value, i, "<(")) && !mainQuote.double && deprecatedArithDepth === 0 && arithDepth === 0) {
              let isProcsub = i === 0 || !/^[a-zA-Z0-9]+$/.test(value[i - 1]) && !"\"'".includes(value[i - 1]);
              if (extglobDepth > 0) {
                j = FindCmdsubEnd(value, i + 2);
                result.push(Substring(value, i, j));
              ...
              if (procsubIdx < procsubParts.length) {
                ...
                let rawContent = Substring(value, i + 2, j - 1);
                if (node.command.kind === "subshell") {
                  let leadingWsEnd = 0;
                  while (leadingWsEnd < rawContent.length && " \t\n".includes(rawContent[leadingWsEnd])) {
                    leadingWsEnd += 1;
                  }
                  let leadingWs = rawContent.slice(0, leadingWsEnd);
                  stripped = rawContent.slice(leadingWsEnd);
                  if (stripped.startsWith("(")) {
                    if ((leadingWs.length > 0)) {
                      let normalizedWs = leadingWs.replace(/\n/g, " ").replace(/\t/g, " ");
                      let spaced = FormatCmdsubNode(node.command, 0, false, false, false);
              ...
              if ((IsExpansionStart(value, i, "${ ") || IsExpansionStart(value, i, "${\t") || IsExpansionStart(value, i, "${\n") || IsExpansionStart(value, i, "${|")) && !IsBackslashEscaped(value, i)) {
                ...
                if (IsExpansionStart(value, i, "${") && !IsBackslashEscaped(value, i)) {
                  ...
                  let formattedInner = this.FormatCommandSubstitutions(inner, false);
  ...
  NormalizeExtglobWhitespace(value) {
    let result = [];
    let i = 0;
    let extglobQuote = newQuoteState();
    let deprecatedArithDepth = 0;
    while (i < value.length) {
      if (value[i] === "\"") {
  ...
  getCondFormattedValue() {
    let value = this.ExpandAllAnsiCQuotes(this.value);
    value = this.StripLocaleStringDollars(value);
    value = this.FormatCommandSubstitutions(value, false);
...
class Pipeline {
  ...
  toSexp() {
    ...
    while (i < this.commands.length) {
      ...
      let needsRedirect = i + 1 < this.commands.length && this.commands[i + 1].kind === "pipe-both";
    ...
    let lastPair = cmds[cmds.length - 1];
    let lastCmd = lastPair[0];
    let lastNeeds = lastPair[1];
    let result = this.CmdSexp(lastCmd, lastNeeds);
    let j = cmds.length - 2;
    while (j >= 0) {
      pair = cmds[j];
      cmd = pair[0];
...
class List {
  ...
  toSexp() {
    ...
    let opNames = new Map([["&&", "and"], ["||", "or"], [";", "semi"], ["\n", "semi"], ["&", "background"]]);
    ...
    if (parts[parts.length - 1].kind === "operator" && parts[parts.length - 1].op === "&") {
      for (let i = parts.length - 3; i > 0; i += -2) {
        if (parts[i].kind === "operator" && (parts[i].op === ";" || parts[i].op === "\n")) {
          let left = Sublist(parts, 0, i);
          let right = Sublist(parts, i + 1, parts.length - 1);
          var leftSexp;
          if (left.length > 1) {
      ...
      let innerParts = Sublist(parts, 0, parts.length - 1);
      if (innerParts.length === 1) {
        return "(background " + innerParts[0].toSexp() + ")";
      }
      let innerList = new List(innerParts, "list");
  ...
  ToSexpWithPrecedence(parts, opNames) {
    ...
    if ((semiPositions.length > 0)) {
      ...
      let result = this.ToSexpAmpAndHigher(segments[0], opNames);
  ...
  ToSexpAmpAndHigher(parts, opNames) {
    ...
    if ((ampPositions.length > 0)) {
      ...
      let result = this.ToSexpAndOr(segments[0], opNames);
      for (let i = 1; i < segments.length; i += 1) {
  ...
  ToSexpAndOr(parts, opNames) {
    ...
    for (let i = 1; i < parts.length - 1; i += 2) {
      let op = parts[i];
      let cmd = parts[i + 1];
      let opName = opNames.get(op.op) ?? op.op;
...
class Operator {
  ...
  toSexp() {
    let names = new Map([["&&", "and"], ["||", "or"], [";", "semi"], ["&", "bg"], ["|", "pipe"]]);
...
class Redirect {
  constructor(op = "", target = null, fd = 0, kind = "") {
    this.op = op;
    this.target = target;
    this.fd = fd;
    this.kind = kind;
  ...
  toSexp() {
    ...
    if (targetVal.startsWith("&")) {
      ...
      let raw = Substring(targetVal, 1, targetVal.length);
      ...
      let fdTarget = (raw.endsWith("-") ? raw.slice(0, raw.length - 1) : raw);
    ...
    if (op === ">&" || op === "<&") {
      ...
      let outVal = (targetVal.endsWith("-") ? targetVal.slice(0, targetVal.length - 1) : targetVal);
...
class HereDoc {
  constructor(delimiter = "", content = "", stripTabs = false, quoted = false, fd = 0, complete = false, StartPos = 0, kind = "") {
...
class Subshell {
  constructor(body = null, redirects = null, kind = "") {
    this.body = body;
    this.redirects = redirects;
    this.kind = kind;
  }
  ...
  toSexp() {
    let base = "(subshell " + this.body.toSexp() + ")";
...
class BraceGroup {
  ...
  toSexp() {
    let base = "(brace-group " + this.body.toSexp() + ")";
    return AppendRedirects(base, this.redirects);
  }
...
class While {
  ...
  toSexp() {
    let base = "(while " + this.condition.toSexp() + " " + this.body.toSexp() + ")";
    return AppendRedirects(base, this.redirects);
  }
...
class Until {
  ...
  toSexp() {
    let base = "(until " + this.condition.toSexp() + " " + this.body.toSexp() + ")";
    return AppendRedirects(base, this.redirects);
  }
...
class For {
  ...
  toSexp() {
    ...
    let tempWord = new Word(this.varName, [], "word");
    let varFormatted = tempWord.FormatCommandSubstitutions(this.varName, false);
    let varEscaped = varFormatted.replace(/\\/g, "\\\\").replace(/"/g, "\\\"");
    if (this.words === null) {
...
class ForArith {
  ...
  toSexp() {
    ...
    let initVal = ((this.init.length > 0) ? this.init : "1");
    let condVal = ((this.cond.length > 0) ? this.cond : "1");
    let incrVal = ((this.incr.length > 0) ? this.incr : "1");
    let initStr = FormatArithVal(initVal);
    let condStr = FormatArithVal(condVal);
    let incrStr = FormatArithVal(incrVal);
    let bodyStr = this.body.toSexp();
...
class Select {
  ...
  toSexp() {
    ...
    let varEscaped = this.varName.replace(/\\/g, "\\\\").replace(/"/g, "\\\"");
    var inClause;
    if (this.words !== null) {
      let wordParts = [];
      wordParts.push(...this.words.map(w => w.toSexp()));
...
class CasePattern {
  ...
  toSexp() {
          ...
          if (IsExpansionStart(this.pattern, i, "$(")) {
            ...
            if (ch === "(" && depth > 0) {
              ...
              if (ch === ")" && depth > 0) {
                ...
                var result1;
                if (ch === "[") {
                  let [result0, result1, result2] = ConsumeBracketClass(this.pattern, i, depth);
                  i = result0;
    ...
    let patternStr = wordList.join(" ");
    let parts = ["(pattern (" + patternStr + ")"];
    if (this.body !== null) {
...
class ParamExpansion {
  ...
  toSexp() {
    let escapedParam = this.param.replace(/\\/g, "\\\\").replace(/"/g, "\\\"");
    if (this.op !== "") {
      let escapedOp = this.op.replace(/\\/g, "\\\\").replace(/"/g, "\\\"");
      var argVal;
      if (this.arg !== "") {
        argVal = this.arg;
      } else {
      ...
      let escapedArg = argVal.replace(/\\/g, "\\\\").replace(/"/g, "\\\"");
...
class ParamLength {
  ...
  toSexp() {
    let escaped = this.param.replace(/\\/g, "\\\\").replace(/"/g, "\\\"");
...
class ArithmeticCommand {
  ...
  toSexp() {
    let formatted = new Word(this.rawContent, null, "word").FormatCommandSubstitutions(this.rawContent, true);
    let escaped = formatted.replace(/\\/g, "\\\\").replace(/"/g, "\\\"").replace(/\n/g, "\\n").replace(/\t/g, "\\t");
    let result = "(arith (word \"" + escaped + "\"))";
    if ((this.redirects.length > 0)) {
      let redirectParts = [];
      redirectParts.push(...this.redirects.map(r => r.toSexp()));
      let redirectSexps = redirectParts.join(" ");
...
class ArithDeprecated {
  ...
  toSexp() {
    let escaped = this.expression.replace(/\\/g, "\\\\").replace(/"/g, "\\\"").replace(/\n/g, "\\n");
...
class AnsiCQuote {
  ...
  toSexp() {
    let escaped = this.content.replace(/\\/g, "\\\\").replace(/"/g, "\\\"").replace(/\n/g, "\\n");
...
class ConditionalExpr {
  ...
  toSexp() {
    ...
    if (typeof body === 'string') {
      let escaped = body.replace(/\\/g, "\\\\").replace(/"/g, "\\\"").replace(/\n/g, "\\n");
      result = "(cond \"" + escaped + "\")";
    } else {
...
class UnaryTest {
  ...
  toSexp() {
    let operandVal = this.operand.getCondFormattedValue();
...
class BinaryTest {
  ...
  toSexp() {
    let leftVal = this.left.getCondFormattedValue();
    let rightVal = this.right.getCondFormattedValue();
...
class Parser {
  ...
  LexPeekToken() {
    ...
    this.SyncLexer();
    let result = this.Lexer.peekToken();
  ...
  LexSkipComment() {
    this.SyncLexer();
    let result = this.Lexer.SkipComment();
    this.SyncParser();
    return result;
  }

  LexIsCommandTerminator() {
    let tok = this.LexPeekToken();
    let t = tok.type;
  ...
  LexIsAtReservedWord(word) {
    let reserved = this.LexPeekReservedWord();
  ...
  IsBangFollowedByProcsub() {
    ...
    let nextChar = this.source[this.pos + 1];
  ...
  CollectRedirects() {
    ...
    while (true) {
      this.skipWhitespace();
      let redirect = this.parseRedirect();
      if (redirect === null) {
        break;
      }
  ...
  ParseLoopBody(context) {
    if (this.peek() === "{") {
      let brace = this.parseBraceGroup();
      if (brace === null) {
    ...
    if (this.LexConsumeWord("do")) {
      let body = this.parseListUntil(new Set(["done"]));
      if (body === null) {
  ...
  consumeWord(expected) {
    ...
    let keywordWord = word;
    let hasLeadingBrace = false;
  ...
  ParseCommandSubstitution() {
    ...
    let text = Substring(this.source, start, textEnd);
    this.RestoreParserState(saved);
  ...
  ParseBacktickSubstitution() {
    ...
    while (!this.atEnd() && (inHeredocBody || this.peek() !== "`")) {
      if (inHeredocBody) {
        ...
        let line = Substring(this.source, lineStart, lineEnd);
        let checkLine = (currentHeredocStrip ? line.replace(/^[\t]+/, '') : line);
        if (checkLine === currentHeredocDelim) {
          ...
          if (checkLine.startsWith(currentHeredocDelim) && checkLine.length > currentHeredocDelim.length) {
            let tabsStripped = line.length - checkLine.length;
            let endPos = tabsStripped + currentHeredocDelim.length;
      ...
      if (c === "<" && this.pos + 1 < this.length && this.source[this.pos + 1] === "<") {
        ...
        let delimiter = delimiterChars.join("");
    ...
    this.advance();
    textChars.push("`");
    let text = textChars.join("");
    let content = contentChars.join("");
    if (pendingHeredocs.length > 0) {
      let [heredocStart, heredocEnd] = FindHeredocContentEnd(this.source, this.pos, pendingHeredocs);
    ...
    let subParser = newParser(content, false, this.Extglob);
    let cmd = subParser.parseList(true);
    if (cmd === null) {
      cmd = new Empty("empty");
    }
    return [new CommandSubstitution(cmd, null, "cmdsub"), text];
  ...
  ParseProcessSubstitution() {
    ...
    this.advance();
    let saved = this.SaveParserState();
    let oldInProcessSub = this.InProcessSub;
    ...
    try {
      ...
      this.RestoreParserState(saved);
      this.InProcessSub = oldInProcessSub;
      let contentStartChar = (start + 2 < this.length ? this.source[start + 2] : "");
  ...
  ParseArrayLiteral() {
    ...
    while (true) {
      ...
      let word = this.parseWord(false, true, false);
      if (word === null) {
        if (this.peek() === ")") {
          break;
        }
  ...
  ParseArithExpr(content) {
    let savedArithSrc = this.ArithSrc;
    let savedArithPos = this.ArithPos;
    let savedArithLen = this.ArithLen;
    let savedParserState = this.ParserState;
  ...
  ArithAdvance() {
    ...
    let c = this.ArithSrc[this.ArithPos];
    this.ArithPos += 1;
    return c;
  }
  ...
  ArithParseComma() {
    let left = this.ArithParseAssign();
    while (true) {
      this.ArithSkipWs();
      if (this.ArithConsume(",")) {
        this.ArithSkipWs();
        let right = this.ArithParseAssign();
  ...
  ArithParseAssign() {
    let left = this.ArithParseTernary();
    this.ArithSkipWs();
    let assignOps = ["<<=", ">>=", "+=", "-=", "*=", "/=", "%=", "&=", "^=", "|=", "="];
  ...
  ArithParseTernary() {
    let cond = this.ArithParseLogicalOr();
    this.ArithSkipWs();
    if (this.ArithConsume("?")) {
      this.ArithSkipWs();
      var ifTrue;
  ...
  ArithParseBitwiseOr() {
    let left = this.ArithParseBitwiseXor();
    while (true) {
      this.ArithSkipWs();
      if (this.ArithPeek(0) === "|" && (this.ArithPeek(1) !== "|" && this.ArithPeek(1) !== "=")) {
        this.ArithAdvance();
        this.ArithSkipWs();
        let right = this.ArithParseBitwiseXor();
  ...
  ArithParseBitwiseXor() {
    let left = this.ArithParseBitwiseAnd();
    while (true) {
      this.ArithSkipWs();
      if (this.ArithPeek(0) === "^" && this.ArithPeek(1) !== "=") {
        this.ArithAdvance();
        this.ArithSkipWs();
        let right = this.ArithParseBitwiseAnd();
        left = new ArithBinaryOp("^", left, right, "binary-op");
      } else {
        break;
      }
  ...
  ArithParseBitwiseAnd() {
    let left = this.ArithParseEquality();
    while (true) {
      this.ArithSkipWs();
      if (this.ArithPeek(0) === "&" && (this.ArithPeek(1) !== "&" && this.ArithPeek(1) !== "=")) {
        this.ArithAdvance();
        this.ArithSkipWs();
        let right = this.ArithParseEquality();
        left = new ArithBinaryOp("&", left, right, "binary-op");
      } else {
        break;
      }
  ...
  ArithParseShift() {
    let left = this.ArithParseAdditive();
    while (true) {
      this.ArithSkipWs();
      if (this.ArithMatch("<<=")) {
        break;
  ...
  ArithParseAdditive() {
    let left = this.ArithParseMultiplicative();
    while (true) {
      this.ArithSkipWs();
      let c = this.ArithPeek(0);
      let c2 = this.ArithPeek(1);
      var right;
      if (c === "+" && (c2 !== "+" && c2 !== "=")) {
        this.ArithAdvance();
        this.ArithSkipWs();
  ...
  ArithParseMultiplicative() {
    let left = this.ArithParseExponentiation();
    while (true) {
      this.ArithSkipWs();
      let c = this.ArithPeek(0);
      let c2 = this.ArithPeek(1);
  ...
  ArithParseExponentiation() {
    let left = this.ArithParseUnary();
    this.ArithSkipWs();
    if (this.ArithMatch("**")) {
      this.ArithConsume("**");
      this.ArithSkipWs();
      let right = this.ArithParseExponentiation();
  ...
  ArithParsePostfix() {
    let left = this.ArithParsePrimary();
    while (true) {
      this.ArithSkipWs();
      if (this.ArithMatch("++")) {
        this.ArithConsume("++");
        ...
        if (this.ArithMatch("--")) {
          ...
          if (this.ArithPeek(0) === "[") {
            if (left instanceof ArithVar) {
              this.ArithAdvance();
              this.ArithSkipWs();
              let index = this.ArithParseComma();
              this.ArithSkipWs();
              if (!this.ArithConsume("]")) {
  ...
  ArithParsePrimary() {
    ...
    if (c === "\\") {
      ...
      let escapedChar = this.ArithAdvance();
  ...
  ArithParseCmdsub() {
    ...
    if (this.ArithPeek(0) === "(") {
      ...
      this.ArithAdvance();
      this.ArithAdvance();
      let innerExpr = this.ParseArithExpr(content);
  ...
  ArithParseSingleQuote() {
    this.ArithAdvance();
    let contentStart = this.ArithPos;
    while (!this.ArithAtEnd() && this.ArithPeek(0) !== "'") {
      this.ArithAdvance();
    }
    let content = Substring(this.ArithSrc, contentStart, this.ArithPos);
    if (!this.ArithConsume("'")) {
  ...
  ArithParseNumberOrVar() {
    ...
    if (/^\d+$/.test(c)) {
      ...
      if (!this.ArithAtEnd() && this.ArithPeek(0) === "$") {
        let expansion = this.ArithParseExpansion();
  ...
  ParseDeprecatedArithmetic() {
    ...
    this.advance();
    this.Lexer.pos = this.pos;
    let content = this.Lexer.ParseMatchedPair("[", "]", MatchedPairFlags_ARITH, false);
    this.pos = this.Lexer.pos;
    let text = Substring(this.source, start, this.pos);
  ...
  ParseParamExpansion(inDquote) {
    this.SyncLexer();
    let [result0, result1] = this.Lexer.ReadParamExpansion(inDquote);
    this.SyncParser();
  ...
  parseRedirect() {
    ...
    if (this.peek() === "{") {
      ...
      let varname = varnameChars.join("");
      let isValidVarfd = false;
      if ((varname.length > 0)) {
        if (/^[a-zA-Z]+$/.test(varname[0]) || varname[0] === "_") {
          if (varname.includes("[") || varname.includes("]")) {
            let left = varname.indexOf("[");
            let right = varname.lastIndexOf("]");
            if (left !== -1 && right === varname.length - 1 && right > left + 1) {
              let base = varname.slice(0, left);
  ...
  ReadHeredocLine(quoted) {
    ...
    if (!quoted) {
      while (lineEnd < this.length) {
        let trailingBs = CountTrailingBackslashes(line);
        if (trailingBs % 2 === 0) {
          break;
        }
  ...
  LineMatchesDelimiter(line, delimiter, stripTabs) {
    let checkLine = (stripTabs ? line.replace(/^[\t]+/, '') : line);
    let normalizedCheck = NormalizeHeredocDelimiter(checkLine);
    let normalizedDelim = NormalizeHeredocDelimiter(delimiter);
  ...
  GatherHeredocBodies() {
    for (const heredoc of this.PendingHeredocs) {
      ...
      while (this.pos < this.length) {
        lineStart = this.pos;
        let [line, lineEnd] = this.ReadHeredocLine(heredoc.quoted);
        let [matches, checkLine] = this.LineMatchesDelimiter(line, heredoc.delimiter, heredoc.stripTabs);
        if (matches) {
          this.pos = (lineEnd < this.length ? lineEnd + 1 : lineEnd);
          break;
        }
        let normalizedCheck = NormalizeHeredocDelimiter(checkLine);
        let normalizedDelim = NormalizeHeredocDelimiter(heredoc.delimiter);
        var tabsStripped;
  ...
  ParseHeredoc(fd, stripTabs) {
    ...
    let [delimiter, quoted] = this.ParseHeredocDelimiter();
    ...
    let heredoc = new HereDoc(delimiter, "", stripTabs, quoted, fd, false, null, "heredoc");
  ...
  parseCommand() {
    ...
    while (true) {
      ...
      let inAssignBuiltin = words.length > 0 && ASSIGNMENT_BUILTINS.has(words[0].value);
      let word = this.parseWord(!(words.length > 0) || allAssignments && redirects.length === 0, false, inAssignBuiltin);
      if (word === null) {
        break;
      }
      words.push(word);
  ...
  parseArithmeticCommand() {
    ...
    let content = Substring(this.source, contentStart, this.pos);
    content = content.replace(/\\\n/g, "");
    this.advance();
    this.advance();
    let expr = this.ParseArithExpr(content);
  ...
  ParseCondTerm() {
    ...
    if (this.peek() === "!") {
      if (this.pos + 1 < this.length && !IsWhitespaceNoNewline(this.source[this.pos + 1])) {
        ...
        this.advance();
        let operand = this.ParseCondTerm();
        return new CondNot(operand, "cond-not");
      }
    }
    if (this.peek() === "(") {
      this.advance();
      let inner = this.ParseCondOr();
      this.CondSkipWhitespace();
      if (this.atEnd() || this.peek() !== ")") {
    ...
    if (COND_UNARY_OPS.has(word1.value)) {
      let unaryOperand = this.ParseCondWord();
  ...
  ParseCondRegexWord() {
    ...
    let result = this.ParseWordInternal(WORD_CTX_REGEX, false, false);
  ...
  parseIf() {
    ...
    let condition = this.parseListUntil(new Set(["then"]));
    if (condition === null) {
    ...
    let thenBody = this.parseListUntil(new Set(["elif", "else", "fi"]));
    if (thenBody === null) {
    ...
    if (this.LexIsAtReservedWord("elif")) {
      ...
      let elifCondition = this.parseListUntil(new Set(["then"]));
      ...
      let elifThenBody = this.parseListUntil(new Set(["elif", "else", "fi"]));
  ...
  parseWhile() {
    ...
    let condition = this.parseListUntil(new Set(["do"]));
    if (condition === null) {
  ...
  parseFor() {
    ...
    var varName;
    if (this.peek() === "$") {
      let varWord = this.parseWord(false, false, false);
      if (varWord === null) {
    ...
    if (this.LexIsAtReservedWord("in")) {
      ...
      this.skipWhitespace();
      let sawDelimiter = IsSemicolonOrNewline(this.peek());
      if (this.peek() === ";") {
        this.advance();
      }
      this.skipWhitespaceAndNewlines();
      ...
      while (true) {
        ...
        let word = this.parseWord(false, false, false);
        if (word === null) {
          break;
        }
        words.push(word);
    ...
    this.skipWhitespaceAndNewlines();
    if (this.peek() === "{") {
      let braceGroup = this.parseBraceGroup();
      if (braceGroup === null) {
  ...
  ParseForArith() {
    ...
    this.skipWhitespaceAndNewlines();
    let body = this.ParseLoopBody("for loop");
  ...
  parseCase() {
    ...
    while (true) {
            ...
            if (IsExpansionStart(this.source, this.pos, "$(")) {
              ...
              if (ch === "(" && extglobDepth > 0) {
                ...
                if (this.Extglob && IsExtglobPrefix(ch) && this.pos + 1 < this.length && this.source[this.pos + 1] === "(") {
                  ...
                  if (ch === "[") {
                    let isCharClass = false;
                    let scanPos = this.pos + 1;
                    let scanDepth = 0;
                    let hasFirstBracketLiteral = false;
      ...
      let terminator = this.ConsumeCaseTerminator();
      this.skipWhitespaceAndNewlines();
  ...
  parseCoproc() {
    ...
    let nextWord = this.LexPeekReservedWord();
    ...
    let wordStart = this.pos;
    let potentialName = this.peekWord();
  ...
  parseFunction() {
    ...
    let posAfterName = this.pos;
    this.skipWhitespace();
    let hasWhitespace = this.pos > posAfterName;
    ...
    while (true) {
      ...
      if (op === "") {
        if (!this.atEnd() && this.peek() === "\n") {
          ...
          let nextOp = this.PeekListOperator();
          if (nextOp === "&" || nextOp === ";") {
            break;
          }
          op = "\n";
  ...
  parsePipeline() {
    ...
    let result = this.ParseSimplePipeline();
    if (prefixOrder === "time") {
  ...
  ParseSimplePipeline() {
    ...
    while (true) {
      this.skipWhitespace();
      let [tokenType, value] = this.LexPeekOperator();
      if (tokenType === 0) {
        break;
      }
      ...
      this.LexNextToken();
      let isPipeBoth = tokenType === TokenType_PIPE_AMP;
      this.skipWhitespaceAndNewlines();
      if (isPipeBoth) {
  ...
  parseListOperator() {
    this.skipWhitespace();
    let [tokenType, ] = this.LexPeekOperator();
    if (tokenType === 0) {
      return "";
    }
  ...
  parse() {
    ...
    while (!this.atEnd()) {
      let result = this.parseList(false);
      if (result !== null) {
        results.push(result);
      }
      this.skipWhitespace();
  ...
  StripTrailingBackslashFromLastWord(nodes) {
    ...
    let lastWord = this.FindLastWord(lastNode);
  ...
  FindLastWord(node) {
    ...
    if (node instanceof Command) {
      if ((node.words.length > 0)) {
        let lastWord = node.words[node.words.length - 1];
        if (lastWord.value.endsWith("\\")) {
          return lastWord;
        }
      }
      if ((node.redirects.length > 0)) {
        let lastRedirect = node.redirects[node.redirects.length - 1];
...
function FormatArithVal(s) {
  let w = new Word(s, [], "word");
  let val = w.ExpandAllAnsiCQuotes(s);
...
function FormatCondBody(node) {
  let kind = node.kind;
  if (kind === "unary-test") {
    let operandVal = node.operand.getCondFormattedValue();
    return node.op + " " + operandVal;
  }
  if (kind === "binary-test") {
    let leftVal = node.left.getCondFormattedValue();
    let rightVal = node.right.getCondFormattedValue();
...
function FormatCmdsubNode(node, indent, inProcsub, compactRedirects, procsubFirst) {
  ...
  let sp = RepeatStr(" ", indent);
  let innerSp = RepeatStr(" ", indent + 4);
  ...
  if (node instanceof Command) {
    ...
    for (const w of node.words) {
      let val = w.ExpandAllAnsiCQuotes(w.value);
      val = w.StripLocaleStringDollars(val);
    ...
    if (compactRedirects && (node.words.length > 0) && (node.redirects.length > 0)) {
      let wordParts = parts.slice(0, node.words.length);
      let redirectParts = parts.slice(node.words.length);
  ...
  if (node instanceof Pipeline) {
    ...
    while (idx < cmds.length) {
      ...
      let formatted = FormatCmdsubNode(cmd, indent, inProcsub, false, procsubFirst && idx === 0);
      let isLast = idx === cmds.length - 1;
    ...
    let compactPipe = inProcsub && (cmds.length > 0) && cmds[0][0].kind === "subshell";
    result = "";
    idx = 0;
    while (idx < resultParts.length) {
      let part = resultParts[idx];
      if (idx > 0) {
        if (result.endsWith("\n")) {
          result = result + "  " + part;
        } else {
  ...
  if (node instanceof List) {
    ...
    for (const p of node.parts) {
      if (p instanceof Operator) {
        ...
        let formattedCmd = FormatCmdsubNode(p, indent, inProcsub, compactRedirects, procsubFirst && cmdCount === 0);
        if (result.length > 0) {
          last = result[result.length - 1];
          if (last.includes(" || \n") || last.includes(" && \n")) {
  ...
  if (node instanceof If) {
    let cond = FormatCmdsubNode(node.condition, indent, false, false, false);
    let thenBody = FormatCmdsubNode(node.thenBody, indent + 4, false, false, false);
    result = "if " + cond + "; then\n" + innerSp + thenBody + ";";
    if (node.elseBody !== null) {
      let elseBody = FormatCmdsubNode(node.elseBody, indent + 4, false, false, false);
      result = result + "\n" + sp + "else\n" + innerSp + elseBody + ";";
    }
    result = result + "\n" + sp + "fi";
    return result;
  ...
  if (node instanceof While) {
    let cond = FormatCmdsubNode(node.condition, indent, false, false, false);
    let body = FormatCmdsubNode(node.body, indent + 4, false, false, false);
  ...
  if (node instanceof Case) {
    ...
    while (i < node.patterns.length) {
      ...
      let patIndent = RepeatStr(" ", indent + 8);
      let termIndent = RepeatStr(" ", indent + 4);
      let bodyPart = ((body.length > 0) ? patIndent + body + "\n" : "\n");
      if (i === 0) {
  ...
  if (node instanceof FunctionName) {
    let name = node.name;
    let innerBody = (node.body.kind === "brace-group" ? node.body.body : node.body);
...
${innerSp}${body}
  ...
  if (node instanceof Time) {
    let prefix = (node.posix ? "time -p " : "time ");
    if (node.pipeline !== null) {
...
function FormatRedirect(r, compact, heredocOpOnly) {
  ...
  if (target.startsWith("&")) {
    let wasInputClose = false;
    if (target === "&-" && op.endsWith("<")) {
      wasInputClose = true;
      op = Substring(op, 0, op.length - 1) + ">";
    }
    let afterAmp = Substring(target, 1, target.length);
    let isLiteralFd = afterAmp === "-" || afterAmp.length > 0 && /^\d+$/.test(afterAmp[0]);
    if (isLiteralFd) {
      if (op === ">" || op === ">&") {
...
function FindBracedParamEnd(value, start) {
  ...
  let i = start;
  let inDouble = false;
  let dolbraceState = DolbraceState_PARAM;
  while (i < value.length && depth > 0) {
    let c = value[i];
    if (c === "\\" && i + 1 < value.length) {
      i += 2;
    ...
    if (c === "[" && dolbraceState === DolbraceState_PARAM && !inDouble) {
      let end = SkipSubscript(value, i, 0);
      if (end !== -1) {
        i = end;
        continue;
      }
...
function SkipHeredoc(value, start) {
  ...
  while (i < value.length) {
    ...
    let line = Substring(value, lineStart, lineEnd);
    ...
    if (stripped.startsWith(delimiter) && stripped.length > delimiter.length) {
      let tabsStripped = line.length - stripped.length;
...
function FindHeredocContentEnd(source, start, delimiters) {
  ...
  for (const Item of delimiters) {
    let delimiter = Item[0];
    let stripTabs = Item[1];
    while (pos < source.length) {
      ...
      let line = Substring(source, lineStart, lineEnd);
      ...
      if (lineStripped.startsWith(delimiter) && lineStripped.length > delimiter.length) {
        let tabsStripped = line.length - lineStripped.length;
...
function SkipMatchedPair(s, start, open, close, flags) {
  ...
  while (i < n && depth > 0) {
    ...
    let literal = flags & _SMP_LITERAL;
    if (!(literal !== 0) && c === "\\") {
      passNext = true;
      i += 1;
      continue;
...
function Assignment(s, flags) {
  ...
  while (i < s.length) {
    ...
    if (c === "[") {
      let subFlags = (((flags & 2) !== 0) ? _SMP_LITERAL : 0);
      let end = SkipSubscript(s, i, subFlags);
      if (end === -1) {
        return -1;
      }
      i = end;
...
function IsArrayAssignmentPrefix(chars) {
  ...
  while (i < s.length) {
    ...
    let end = SkipSubscript(s, i, _SMP_LITERAL);
    if (end === -1) {
      return false;
    }
    i = end;
...
function parse(source, extglob) {
  let parser = newParser(source, false, extglob);
  return parser.parse();
}

function newParseError(message, pos, line) {
  let self = new ParseError(null, null, null);
...
function newQuoteState() {
  let self = new QuoteState(null, null, null);
  self.single = false;
  self.double = false;
  self.Stack = [];
  return self;
...
function newParseContext(kind) {
  let self = new ParseContext(null, null, null, null, null, null, null, null);
  self.kind = kind;
  self.parenDepth = 0;
  self.braceDepth = 0;
  self.bracketDepth = 0;
...
function newContextStack() {
  let self = new ContextStack(null);
  self.Stack = [newParseContext(0)];
  return self;
}

function newLexer(source, extglob) {
  let self = new Lexer(null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null);
  self.source = source;
  self.pos = 0;
...
function newParser(source, inProcessSub, extglob) {
  let self = new Parser(null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null);
  self.source = source;
  self.pos = 0;
  self.length = source.length;
  self.PendingHeredocs = [];
...
if (typeof module !== 'undefined') {
  module.exports = { parse, newParseError, newMatchedPairError, newQuoteState, newParseContext, newContextStack, newLexer, newParser, ParseError, MatchedPairError, TokenType, Token, ParserStateFlags, DolbraceState, MatchedPairFlags, SavedParserState, QuoteState, ParseContext, ContextStack, Lexer, Word, Command, Pipeline, List, Operator, PipeBoth, Empty, Comment, Redirect, HereDoc, Subshell, BraceGroup, If, While, Until, For, ForArith, Select, Case, CasePattern, FunctionName, ParamExpansion, ParamLength, ParamIndirect, CommandSubstitution, ArithmeticExpansion, ArithmeticCommand, ArithNumber, ArithEmpty, ArithVar, ArithBinaryOp, ArithUnaryOp, ArithPreIncr, ArithPostIncr, ArithPreDecr, ArithPostDecr, ArithAssign, ArithTernary, ArithComma, ArithSubscript, ArithEscape, ArithDeprecated, ArithConcat, AnsiCQuote, LocaleString, ProcessSubstitution, Negation, Time, ConditionalExpr, UnaryTest, BinaryTest, CondAnd, CondOr, CondNot, CondParen, ArrayName, Coproc, Parser };
}
