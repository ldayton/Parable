...
-- Timeout via wall-clock time (10 seconds)
...
local function run_with_timeout(func)
  ...
  debug.sethook(hook, "", 1000)  -- Hook every 1000 VM instructions to check time
...
local function main()
  ...
  while i <= #arg do
    ...
    elseif a == "--max-failures" then
      i = i + 1
      if i <= #arg then
        max_failures = tonumber(arg[i])
      end
    else
      test_dir = a
    end
  ...
  if total_failed > 0 then
    ...
    print("FAILURES")
    print(string.rep("=", 60))
    local show_count = max_failures == 0 and #failed_tests or math.min(#failed_tests, max_failures)
    for j = 1, show_count do
      local f = failed_tests[j]
    ...
    if max_failures > 0 and total_failed > max_failures then
      print(string.format("\n... and %d more failures", total_failed - max_failures))
    end
    print("")
  end
