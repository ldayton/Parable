...
class Token {
    constructor(typeName = 0, value = "", pos = 0, parts = [], word = null) {
                    ...
                    if (nextCh === "(") {
                        ...
                        if (this.pos + 2 < this.length && this.source[this.pos + 2] === "(") {
                            ...
                            if (arithNode !== null) {
                                ...
                                this.SyncToParser();
                                var [cmdNode, cmdText] = this.Parser.ParseCommandSubstitution();
                                this.SyncFromParser();
                                if (cmdNode !== null) {
                                    chars.push(cmdText);
                                    wasDollar = false;
                        ...
                        if (nextCh === "[") {
                            this.pos -= 1;
                            this.SyncToParser();
                            var [arithNode, arithText] = this.Parser.ParseDeprecatedArithmetic();
                            this.SyncFromParser();
                            if (arithNode !== null) {
                                chars.push(arithText);
                                wasDollar = false;
                    ...
                    while (!this.atEnd() && (inSingleInDquote || this.peek() !== "\"")) {
                        ...
                        if (c === "\\" && this.pos + 1 < this.length) {
                            ...
                            if (c === "$") {
                                ...
                                if (c === "`") {
                                    this.SyncToParser();
                                    var [cmdsubResult0, cmdsubResult1] = this.Parser.ParseBacktickSubstitution();
                                    this.SyncFromParser();
                ...
                if (ch === "\\" && this.pos + 1 < this.length) {
                    ...
                    if (ch === "$" && this.pos + 2 < this.length && this.source[this.pos + 1] === "(" && this.source[this.pos + 2] === "(") {
                        ...
                        if (arithNode !== null) {
                            ...
                            this.SyncToParser();
                            var [cmdsubNode, cmdsubText] = this.Parser.ParseCommandSubstitution();
                            this.SyncFromParser();
                            if (cmdsubNode !== null) {
                        ...
                        if (IsExpansionStart(this.source, this.pos, "$(")) {
                            ...
                            if (ch === "$") {
                                ...
                                if (ch === "`") {
                                    this.SyncToParser();
                                    var [cmdsubNode, cmdsubText] = this.Parser.ParseBacktickSubstitution();
                                    this.SyncFromParser();
                                    if (cmdsubNode !== null) {
                                        innerParts.push(cmdsubNode);
                                        contentChars.push(cmdsubText);
...
class CasePattern {
    ...
    toSexp() {
                        ...
                        if (ch === "(" && depth > 0) {
                            ...
                            if (ch === ")" && depth > 0) {
                                ...
                                if (ch === "[") {
                                    ...
                                    if (ch === "'" && depth === 0) {
                                        var [result0, result1] = ConsumeSingleQuote(this.pattern, i);
                                        i = result0;
                                        current.push(...result1);
                                    }
                                    else {
                                        if (ch === "\"" && depth === 0) {
                                            var [result0, result1] = ConsumeDoubleQuote(this.pattern, i);
                                            i = result0;
                                            current.push(...result1);
                                        }
                                        else {
        ...
        alternatives.push(current.join(""));
        var wordList = [];
        wordList.push(...alternatives.map(alt => new Word(alt, [], "word").toSexp()));
...
class Parser {
    ...
    ParseDollarExpansion(chars, parts, inDquote) {
        if (this.pos + 2 < this.length && this.source[this.pos + 1] === "(" && this.source[this.pos + 2] === "(") {
            var [result0, result1] = this.ParseArithmeticExpansion();
            if (result0 !== null) {
                parts.push(result0);
                chars.push(result1);
                return true;
        ...
        if (this.pos + 1 < this.length && this.source[this.pos + 1] === "[") {
            var [result0, result1] = this.ParseDeprecatedArithmetic();
            if (result0 !== null) {
                parts.push(result0);
                chars.push(result1);
                return true;
        ...
        if (this.pos + 1 < this.length && this.source[this.pos + 1] === "(") {
            var [result0, result1] = this.ParseCommandSubstitution();
            if (result0 !== null) {
                parts.push(result0);
                chars.push(result1);
                return true;
        ...
        var [result0, result1] = this.ParseParamExpansion(inDquote);
        if (result0 !== null) {
            parts.push(result0);
            chars.push(result1);
            return true;
