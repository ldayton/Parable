...
export class ParseError extends Error {
	...
	constructor(message: string, pos?: number | null, line?: number | null);
}
...
// Word node (fundamental building block)
...
// Union type for all nodes (discriminated union via kind property)
export type Node =
	// Structural
	| Word
	| Command
 ...
 * @example
 * ```typescript
 * import { parse } from 'parable';
 *
 * const ast = parse('echo hello | grep h');
 * console.log(ast[0].kind); // 'pipeline'
 * ```
 */
