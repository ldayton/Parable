...
def main():
    ...
    def add_common_args(p, has_iterations=True):
        if has_iterations:
            p.add_argument("-n", "--iterations", type=int, default=1000, help="number of iterations (default: 1000)")
        p.add_argument("-o", "--output", type=Path, metavar="FILE", help="output file for discrepancies")
        p.add_argument("-v", "--verbose", action="store_true", help="verbose output")
        p.add_argument("--both-succeed", action="store_true", help="only show cases where both parsers succeed but differ")
        p.add_argument("--stop-after", type=int, metavar="N", help="stop after finding N unique discrepancies")
        p.add_argument("--minimize", action="store_true", help="minimize discrepancies before output")
        p.add_argument("--filter-layer", metavar="SPEC", help="only show discrepancies at or below this layer (implies --minimize)")

    # Character mode
    char_parser = subparsers.add_parser(
        "character", aliases=["char"],
        help="character mutation fuzzer",
        description=CHAR_DESC,
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    add_common_args(char_parser)
    char_parser.add_argument("-m", "--mutations", type=int, default=2, help="mutations per input (default: 2)")
    char_parser.add_argument("-s", "--seed", type=int, help="random seed")

    # Structural mode
    struct_parser = subparsers.add_parser(
        "structural", aliases=["struct"],
        help="structural transformation fuzzer",
        description=STRUCT_DESC,
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    add_common_args(struct_parser, has_iterations=False)
    struct_parser.add_argument("--transforms", metavar="LIST", help="comma-separated transforms (default: all)")
    struct_parser.add_argument("--list-transforms", action="store_true", help="list available transforms and exit")

    # Generator mode
    gen_parser = subparsers.add_parser(
        "generator", aliases=["gen"],
    ...
    gen_parser.add_argument("--layer", default="full", metavar="SPEC", help="layer spec: 0-12, range (0-5), or preset (words, commands, full)")
    gen_parser.add_argument("--max-depth", type=int, default=5, help="max recursion depth (default: 5)")
    gen_parser.add_argument("--list-layers", action="store_true", help="list available layers and exit")
    gen_parser.add_argument("--dry-run", type=int, metavar="N", help="generate N samples without testing")

    # Minimize mode
    min_parser = subparsers.add_parser(
        "minimize", aliases=["min"],
    ...
    min_parser.add_argument("-t", "--timeout", type=int, default=10, help="timeout in seconds (default: 10)")
